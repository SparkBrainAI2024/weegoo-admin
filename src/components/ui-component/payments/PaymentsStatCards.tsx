...
import { PaymentsPeriod, usePaymentsSummary } from 'graphql/queries/payments.queries';

const formatCurrency = (value: number) => `Rs. ${value.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

export default function PaymentsStatCards({ period }: { period: PaymentsPeriod }) {
    const { data, loading } = usePaymentsSummary(period);
